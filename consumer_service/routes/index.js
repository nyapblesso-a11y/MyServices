...
router.get("/", async function (req, res, next) {
  ...
  const bicycleReq = await fetch(`${bicycleService}/bicycle/`);
  const brandReq = await fetch(`${brandService}/brand/`);
  ...
  const bicycles = results[0].value.data;
  const brands = results[1].value.data;

  const combined = bicycles.map((bike) => {
    const brandMatch = brands.find((b) => b.id === bike.id);

    return {
      id: bike.id,
      color: bike.color,
      brand: brandMatch ? brandMatch.brand : null,
    };
  });
